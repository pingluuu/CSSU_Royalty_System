...
const updateCurrentUser = async (req, res) => {
    try {
        ...
        const requestUser = req.user; // Get user info from authentication middleware
        const { name, email, birthday } = req.body;
        const avatar = req.file ? `/uploads/avatars/${req.file.filename}` : null; // Avatar URL if uploaded

        const updatedUser = await userService.updateCurrentUser(userId, { name, email, birthday, avatarUrl: avatar }, requestUser);
